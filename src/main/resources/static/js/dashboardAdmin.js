...
const datosReservasLocal = [
  ...
  {
    id: 5,
    nombreHuesped: "Carlos Mendez",
    iniciales: "CM",
    fechaHora: "2024-10-25 21:00",
    cantidadPersonas: 5,
    numeroMesa: "L-01",
    estado: "PENDIENTE",
    tipoEvento: "BIRTHDAY",
...
const datosMesasLocal = [
  ...
  },
  {
    ...
    numeroMesa: "L-02",
    capacidad: 4,
    nombreUbicacion: "Comedor Principal",
    activa: false,
    disponible: false,
  },
]
...
async function cargarDatosDelBackend() {
  ...
  try {
    ...
    const resReservas = await fetch(
      `${API_BASE}/reservas-recientes?pagina=0&tamanio=9999`,
      {
        headers: {
          Authorization: `Bearer ${TOKEN}`,
          "Content-Type": "application/json",
...
async function actualizarEstadisticas() {
  ...
  // Cálculo local
  const mesasDisponibles = datosMesas.filter((m) =>
    m.disponible !== undefined ? m.disponible : m.activa,
  ).length
  ...
  const totalHuespedes = datosReservas.reduce(
    (s, r) => s + (r.cantidadPersonas || r.numeroPersonas || 0),
    0,
  )
  const pendientes = datosReservas.filter(
...
function cargarReservasEnTabla() {
  ...
  if (total === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align:center;color:#999;padding:40px;">No hay reservas registradas.</td></tr>'
  } else {
...
function crearFilaReserva(r) {
  ...
  const iniciales =
    r.iniciales || (r.nombreHuesped || "U").substring(0, 2).toUpperCase()
...
function cargarMesasEnTabla() {
  ...
  if (todasLasMesas.length === 0) {
    container.innerHTML =
      '<p style="color:#999;text-align:center;padding:20px;">No hay mesas configuradas.</p>'
    return
  }
...
function actualizarBadgesMesas() {
  const activas = todasLasMesas.filter((m) =>
...
function configurarFormularioMesas() {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    const numeroDuplicado = todasLasMesas.find(
      (m) =>
    ...
    if (numeroDuplicado) {
      alert(
        `⚠️ Ya existe una Mesa con el número ${numero}. Usa un número diferente.`,
      )
      return
    }
    ...
    const nombreInput = document
      .getElementById("numeroMesa")
      .value.trim()
      .toLowerCase()
    const nombreDuplicado = todasLasMesas.find(
      (m) =>
    ...
    if (nombreDuplicado) {
      alert(
        `⚠️ Ya existe una Mesa con el nombre "${document.getElementById("numeroMesa").value.trim()}". Usa un nombre diferente.`,
      )
      return
    }
    ...
    if (conectadoAlBackend && TOKEN) {
      try {
        const url = mesaIdParaEditar
          ? `${API_BASE}/mesas/${mesaIdParaEditar}`
          : `${API_BASE}/mesas`
...
function editarMesa(mesaId) {
  ...
  const radioASeleccionar = document.querySelector(
    `input[name="sector"][value="${sectorValor}"]`,
  )
  if (radioASeleccionar) radioASeleccionar.checked = true
  document.getElementById("activa").checked =
    mesa.disponible !== undefined
      ? mesa.disponible
      : mesa.activa !== undefined
        ? mesa.activa
        : true
...
function renderizarTablaReservasTodas(lista) {
  ...
  if (datos.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7" style="text-align:center;color:#999;padding:40px;">No se encontraron reservas.</td></tr>'
    return
  }
...
function filtrarReservasGlobales() {
  ...
  const filtradas = datosReservas.filter((r) => {
    const matchTexto =
      !termino ||
      (r.nombreHuesped || "").toLowerCase().includes(termino) ||
      (r.fechaHora || "").includes(termino)
...
async function eliminarReservaAdmin(id) {
  ...
  datosReservasTodas = datosReservasTodas.filter(
...
async function cargarUsuariosSistema() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="6" style="text-align:center;color:#999;padding:30px;">Cargando usuarios desde MongoDB...</td></tr>'

  if (!TOKEN) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align:center;color:#f44336;padding:30px;">Sin autenticación. Inicia sesión.</td></tr>'
    return
  }
  ...
  try {
    ...
    if (usuarios.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align:center;color:#999;padding:30px;">No hay usuarios registrados.</td></tr>'
      return
    }
    ...
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align:center;color:#f44336;padding:30px;">Error al conectar con el servidor.</td></tr>'
  }
}
...
function configurarBusquedaUsuarios(usuarios) {
  ...
  function filtrar() {
    ...
    const filtrados = usuarios.filter((u) => {
      ...
      return (
        (!termino || nombre.includes(termino) || correo.includes(termino)) &&
        (!rol || u.rol === rol)
      )
    })
    ...
    if (filtrados.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align:center;color:#999;padding:30px;">Sin resultados.</td></tr>'
      return
    }
    filtrados.forEach((u) => {
      const rolColor =
...
function configurarBusquedas() {
  ...
  if (buscarHuesped) {
    buscarHuesped.addEventListener("input", function () {
      ...
      document.querySelectorAll("#reservasTableBody tr").forEach((tr) => {
        tr.style.display = tr.textContent.toLowerCase().includes(termino)
          ? ""
          : "none"
      })
    })
  ...
  if (buscarReserva)
    buscarReserva.addEventListener("input", filtrarReservasGlobales)
  if (filtroEstado)
    filtroEstado.addEventListener("change", filtrarReservasGlobales)
}
...
function filtrarMesas() {
  ...
  if (filtradas.length === 0) {
    container.innerHTML =
      '<p style="color:#999;text-align:center;padding:20px;">No se encontró ninguna mesa.</p>'
    return
  }
